...
def location_calculator(current_perimeter_level, previous_perimeter_end, input_value):
    ...
    for _ in range(1, difference + 1):
        if movement_counter <= distance_to_top:
            perimeter_start_location[1] += 1

        elif movement_counter <= distance_to_top + distance_across_sides:
            perimeter_start_location[0] -= 1

        elif movement_counter <= distance_to_top + distance_across_sides * 2:
            perimeter_start_location[1] -= 1
